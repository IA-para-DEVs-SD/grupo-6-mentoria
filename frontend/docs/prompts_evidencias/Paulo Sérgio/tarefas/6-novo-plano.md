...
Reutilização inteligente do fluxo de onboarding existente para gerar novos planos, com mudanças mínimas.

**Pontos fortes:**

- IA identificou que a OnboardingPage já pré-preenchia dados do perfil — apenas adicionou detecção de modo
- Mudança mínima: apenas label do botão e detecção de `?mode=new-plan` na URL
- Reutilização máxima de código existente — princípio DRY aplicado pela IA
