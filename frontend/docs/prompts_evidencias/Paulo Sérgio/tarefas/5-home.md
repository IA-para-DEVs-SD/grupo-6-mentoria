...
HomePage com listagem de planos e ações CRUD. A IA otimizou a store para filtrar localmente no delete em vez de re-fetchar.

**Pontos fortes:**

- Otimização proativa: mudou `deletePlan` para filtrar localmente em vez de recarregar toda a lista
- Implementou confirmação de delete via ConfirmDialog do PrimeVue
- EmptyState component para quando não há planos
